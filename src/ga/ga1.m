...
lb = -10;   % 变量的下界
ub = 10;    % 变量的上界
nvars = 1;  % 变量个数（对于本问题是1）

% 目标函数（即适应度函数）
fitness_function = @(x) x.^2 - 4.*x + 4;

% 初始化种群：种群为一个矩阵，每行代表一个个体
...
for gen = 1:max_gen
    ...
    for i = 1:size(offspring, 1)
        if rand < mutation_rate  % 以一定概率进行变异
            mutation_point = randi(nvars);
            mutation_value = lb + (ub - lb) * rand;
    ...
    fprintf('Generation %d: Best solution = %.4f, Fitness = %.4f\n', gen, best_solution, best_fitness);
end

% 最终最优解
best_solution = population(1, :);
best_fitness = fitness_function(best_solution);
fprintf('Final best solution = %.4f, Fitness = %.4f\n', best_solution, best_fitness);
