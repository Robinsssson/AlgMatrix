...
void test_pso() {
    ...
    for (int iter = 0; iter < max_iterations; iter++) {
        ...
        // 输出每次迭代的最优解和最优适应度值
        printf("Iteration %d: Best fitness = %f, Best position = (%f, %f)\n", iter, pso->g_best_fitness, pso->g_best->mat[0], pso->g_best->mat[1]);
    }

    // 检查最终结果
    printf("Final best fitness = %f, Final best position = (%f, %f)\n", pso->g_best_fitness, pso->g_best->mat[0], pso->g_best->mat[1]);
